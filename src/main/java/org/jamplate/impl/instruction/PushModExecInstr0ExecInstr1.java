...
/**
 * <h3>{@code PUSH( MOD( EXEC( INSTR0 ) , EXEC( INSTR1 ) ) ) )}</h3>
 * Pushes the results of applying "modulo" to the results of executing two pre-specified
 * instructions.
 *
 * @author LSafer
 * @version 0.2.0
 ...
 */
@Deprecated
public class PushModExecInstr0ExecInstr1 implements Instruction {
	@SuppressWarnings("JavaDoc")
	private static final long serialVersionUID = -7789981405934742759L;
	 ...
	 * result of executing the given {@code instruction0} with the results of executing
	 * the given {@code instruction1}.
	 *
	 ...
	 * @since 0.2.0 ~2021.06.03
	 */
	public PushModExecInstr0ExecInstr1(@NotNull Instruction instruction0, @NotNull Instruction instruction1) {
	 ...
	 * @since 0.2.0 ~2021.06.03
	 */
	public PushModExecInstr0ExecInstr1(@NotNull Tree tree, @NotNull Instruction instruction0, @NotNull Instruction instruction1) {
	...
	public void exec(@NotNull Environment environment, @NotNull Memory memory) {
		...
		//MOD( EXEC( INSTR0 ) , EXEC( INSTR1 ) )
		String text0 = value0.evaluate(memory);
		String text1 = value1.evaluate(memory);
		String text3;

		try {
			double num0 = Values.toNumber(text0);
			double num1 = Values.toNumber(text1);
			...
			double num3 = num0 % num1;

			if (num3 % 1 == 0)
				text3 = Long.toString((long) num3);
			else
				text3 = Double.toString(num3);
		} catch (NumberFormatException ignored) {
			throw new ExecutionException(
					"The operator (%) accepts numbers only!",
					this.tree
			);
		}
