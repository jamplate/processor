 ...
 */
public final class CurlyBracketsSketch extends AbstractContextSketch {
	/**
	...
	public static class SketcherVisitor implements Sketch.Visitor {
		...
		public boolean visit(Sketch sketch) {
			...
			Reference reference = Sketch.find(
					sketch,
					CurlyBracketsSketch.PATTERN_START,
					CurlyBracketsSketch.PATTERN_END
			);

			if (reference != null) {
				Sketch brackets = new CurlyBracketsSketch(reference);
				while (brackets.accept(CurlyBracketSketch.VISITOR))
					//parse the bracket
					;
				sketch.put(brackets);
				return true;
