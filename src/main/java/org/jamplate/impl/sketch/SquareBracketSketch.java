 ...
 */
public final class SquareBracketSketch extends AbstractConcreteSketch {
	/**
	...
	/**
	 * A visitor that parses available matching bracket into {@link SquareBracketSketch}.
	 * <br>
	...
	public static class SketcherVisitor implements Sketch.Visitor {
		...
		public boolean visit(Sketch sketch) {
			...
			if (sketch instanceof SquareBracketsSketch) {
				Reference reference = Sketch.find(
						sketch,
						SquareBracketSketch.PATTERN
				);

				if (reference != null) {
					Sketch parenthesis = new SquareBracketSketch(reference);
