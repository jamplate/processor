...
/**
 * Source-code Input/Output related functions.
 *
 * @author LSafer
 * @version 0.3.0
...
public final class Source {
	...
	@Contract(pure = true)
	public static int linePosition(@NotNull Tree tree) {
		Objects.requireNonNull(tree, "tree");
		int p = tree.getReference().position();
		try (LineNumberReader reader = new LineNumberReader(tree.getDocument().openReader())) {
			char[] b = new char[2];
	...
	/**
	 * Try to get the line of the given {@code tree} on its document.
	 *
	 * @param tree the tree to try to get its line.
	 * @return the line of the given {@code tree}, or {@code 0} if failed to get its line.
	 * @throws NullPointerException if the given {@code tree} is null.
	 * @since 0.3.0 ~2021.07.04
	 */
	@Contract(pure = true)
	public static int optLine(@NotNull Tree tree) {
		try {
			return Source.line(tree);
	...
	/**
	 * Try to get the position of the given {@code tree} on its line.
	 *
	 * @param tree the tree to try to get its position in its line.
	 * @return the position of the given {@code tree} on its line, or {@code 0} if fail to
	 * 		get its position.
	 * @throws NullPointerException if the given {@code tree} is null.
	 * @since 0.3.0 ~2021.07.04
	 */
	@Contract(pure = true)
	public static int optLinePosition(@NotNull Tree tree) {
		try {
			return Source.linePosition(tree);
		} catch (IOError ignored) {
			return 0;
		}
	}
	...
	/**
	 * Try to read the source-code of the given {@code tree}.
	 *
	 * @param tree the tree to try to read its source-code.
	 * @return the source code of the given {@code tree}, or an empty string if failed to
	 * 		read it.
	 * @throws NullPointerException if the given {@code tree} is null.
	 * @since 0.3.0 ~2021.07.04
	 */
	@NotNull
	@Contract(pure = true)
	public static CharSequence optRead(@NotNull Tree tree) {
		try {
			return Source.read(tree);
		} catch (IOError ignored) {
			return "";
		}
	}
	...
	/**
	 * Try to read the whole line that the given {@code tree} is at on its document.
	 *
	 * @param tree the tree to try to read the line it is in.
	 * @return the text of the line the given {@code tree} is at, or an empty string if
	...
	@NotNull
	@Contract(pure = true)
	public static CharSequence optReadLine(@NotNull Tree tree) {
		try {
			return Source.readLine(tree);
		} catch (IOError ignored) {
			return "";
		}
	}
