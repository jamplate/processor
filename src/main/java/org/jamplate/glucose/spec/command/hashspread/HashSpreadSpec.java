...
import org.jamplate.glucose.instruction.operator.struct.ISpread;
...
/**
 * The specification of the command {@code #spread}.
 *
 * @author LSafer
 * @version 0.3.0
 * @since 0.3.0 ~2021.07.10
 */
public class HashSpreadSpec implements Spec {
	/**
	 ...
	 */
	@NotNull
	public static final HashSpreadSpec INSTANCE = new HashSpreadSpec();

	/**
	 * The kind of the {@code #spread} command.
	 *
	 * @since 0.3.0 ~2021.07.10
	 */
	@NotNull
	public static final String KIND = "command:spread";
	 ...
	 */
	@NotNull
	public static final String NAME = HashSpreadSpec.class.getSimpleName();

	@NotNull
	@Override
	public Compiler getCompiler() {
		return compiler(
				//target spread commands
				c -> filter(c, is(HashSpreadSpec.KIND)),
				//compile
				c -> (compiler, compilation, tree) -> {
					...
					if (valueT == null)
						throw new CompileException(
								"Command SPREAD is missing some components",
								tree
						);
		...
		return parser(
				p -> group(
						...
						(d, r) -> new Tree(
								d,
								r,
								new Sketch(HashSpreadSpec.KIND),
								CommandSpec.WEIGHT
						),
